...
export default function Component() {
  ...
  const handleAiReplyClick = () => {
    ...
    누가 : ${who} || 누구에게 : ${recipient} || 언제 : ${when} || 어디서 : ${where} || 무엇을 :${what} || 어떻게 : ${how} 
    || 왜 : ${why} || 문서를 생성할 언어: ${lan}`;
    console.log(content);
    setRunGPT(true);
    ...
    <ContainerCenter>
      <PageContainer>
        <CustomColumn width='80%' gap='4rem' justifyContent='center' alignItems='center'>
          ...
          {showTicketAlert && (
            <SoldOutFreeTicket onClose={() => setShowTicketAlert(false)} onTicketUsed={handleTicketUsed} category="notice" />
          )}
